# Redis Rate Limiter Specification

## Purpose
Provides self-defending rate limiting capabilities across all HTTP ingress endpoints in the `store_order` microservice using Redis-backed sliding window counters, protect sensitive checkout and cancellation routes from abuse, and gracefully fail open during cache outages.
