...
The system SHALL persist all outbound domain events (`order.created`, `order.paid`, `order.cancelled`, `order.shipped`) into the PostgreSQL `outbox_events` table within the same ACID database transaction that modifies the order state, targeted to the `order.events` Kafka topic with standardized envelope (`event_id`, `event_type`, `timestamp`, `producer: "store_order"`, `data`).
